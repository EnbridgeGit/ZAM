REPORT ZAAMR003 NO STANDARD PAGE HEADING LINE-SIZE 255 LINE-COUNT 65.
************************************************************************
* Program     : ZAAMR003
...
* Date        : May 1997.
*
* Description : Displays the ASSET MANAGEMENT/PIPE SIZE QUERY
...
*     anln1                like anla-anln1,         "Asset Number
      ...
      CURBAL               LIKE ANLC-KANSW,         "Acquisition Value
...
SELECT-OPTIONS:
      S_ORD41     FOR ANLA-ORD41.       "Pipe Size
...
SELECT * FROM ANLA                     "All other details for report
    WHERE ORD41 IN S_ORD41
    AND BUKRS = P_BUKRS.

    PERFORM CREATE_TABLE.
ENDSELECT.

SORT: TABLE1 BY ORD41 ANLN1 ANLN2.
...
FORM CREATE_TABLE.
   ...
   SELECT * FROM ANLC
        ...
        TABLE1-CURBAL = ANLC-KANSW + ANLC-ANSWL
